...
const (
	...
	LABE_TEMPLATE         = "%s\n"
	//发送消息使用的url
	SEND_APP_MSG_URL = `https://qyapi.weixin.qq.com/cgi-bin/appchat/send?access_token=`
	//获取token使用的url
...
func (d *WechatSink) Send(event *v1.Event) {
	...
	resp, err := http.Post(SEND_APP_MSG_URL+token.AccessToken, CONTENT_TYPE_JSON, b)

	if err != nil {
		klog.Errorf("failed to send msg to wechatgroup. error: %s", err.Error())
		return
	}
	if resp != nil && resp.StatusCode != http.StatusOK {
		klog.Errorf("failed to send msg to wechatgroup, because the response code is %d", resp.StatusCode)
		return
	}
...
func NewWechatGroupSink(uri *url.URL) (*WechatSink, error) {
	d := &WechatSink{
		Level: WARNING,
	}
	opts := uri.Query()
	...
	if len(opts["chat_id"]) >= 1 {
		d.ChatID = opts["chat_id"][0]
	} else {
		return nil, fmt.Errorf("you must provide wechat chatid")
	}
